...
import os
data_dir = 'DataFiles'
data_file_path = os.path.join(os.getcwd(), data_dir, 'data', 'fciannel')
data_file = os.path.join(data_file_path, 'data.mat')
...
samples_per_song = sample_frequency * seconds_per_sample

C = io.loadmat(data_file)

data_stripped = C['data'][0][0]
print(data_stripped.shape)
timeLength = (data_stripped.shape[0])/256.0
print(timeLength)
events = data_stripped[:,44]
events_indices = np.where(events != 0)
events_indices = events_indices[0]
...
all_songs = np.zeros((samples_per_song, n_chans, n_samples))

index = 0
for i in range(n_samples):
    index = index + 1

    from_marker = events_indices[i]
    to_end_of_sample = from_marker + samples_per_song

    new_data = data_stripped[from_marker:to_end_of_sample,:]
    new_data_arranged = np.concatenate((new_data[:, left], new_data[:, right], new_data[:, middle]), axis=1)
    all_songs[:,:,index] = new_data_arranged
    print(1)
"""
...
sizebin = 1.5; % sec window
overlap = 0.625; % sec overlap between windows
